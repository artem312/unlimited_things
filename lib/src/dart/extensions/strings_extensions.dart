...
extension PathFormatExtension on String {
  ...
  String get pathEncoded => replaceAll('/', Platform.pathSeparator).replaceAll(r'\', Platform.pathSeparator);
}
...
extension SttrinFormatExtension on String {
  ...
  String get capitalize => length < 2 ? toUpperCase() : '${this[0].toUpperCase()}${substring(1)}';

  String get uncapitalize => length < 2 ? toLowerCase() : '${this[0].toLowerCase()}${substring(1)}';
  ...
  String get snakeCase =>
      uncapitalize.splitMapJoin(RegExp(r'([A-Z|\s])'), onMatch: (m) => '_${m[1]}'.trim()).toLowerCase();
}
